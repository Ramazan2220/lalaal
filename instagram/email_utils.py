...
def get_code_from_firstmail_with_imap_tools(email, password, max_attempts=3, delay_between_attempts=5):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            print(f"[DEBUG] Попытка {attempt+1} получения кода с использованием imap_tools")
            ...
            with MailBox('imap.firstmail.ltd', 993).login(email, password) as mailbox:
                ...
                messages = mailbox.fetch(
                    AND(from_='security@mail.instagram.com'),
                    limit=10,
                    reverse=True
                )

                messages_list = list(messages)
                print(f"[DEBUG] Найдено {len(messages_list)} писем от Instagram")

                for msg in messages_list:
                    print(f"[DEBUG] Проверяем письмо: {msg.subject}")
                    ...
                    # Выводим больше информации о содержимом письма
                    print(f"[DEBUG] Текст письма (первые 300 символов): {message_content[:300]}")

                    # Различные шаблоны для поиска кода
                    patterns = [
                        r'(\d{6})',  # Простой 6-значный код
                        r'[Cc]ode:?\s*(\d{6})',  # "Code: XXXXXX"
                        r'[Cc]ode\s+is\s+(\d{6})',  # "Code is XXXXXX"
                        r'[Ss]ecurity\s+[Cc]ode:?\s*(\d{6})',  # "Security Code: XXXXXX"
                        r'[Vv]erification\s+[Cc]ode:?\s*(\d{6})',  # "Verification Code: XXXXXX"
                        r'[Uu]se\s+(?:the\s+)?(?:following\s+)?[Cc]ode:?\s*(\d{6})',  # "Use the following code: XXXXXX"
                        r'[Uu]se\s+(?:the\s+)?(?:following\s+)?(\d{6})',  # "Use the following XXXXXX"
                        r'[Ee]nter\s+(?:the\s+)?(?:following\s+)?[Cc]ode:?\s*(\d{6})',  # "Enter the following code: XXXXXX"
                        r'[Ee]nter\s+(?:the\s+)?(?:following\s+)?(\d{6})',  # "Enter the following XXXXXX"
                    ]

                    # Проверяем каждый шаблон
                    for pattern in patterns:
                        code_match = re.search(pattern, message_content)
                        if code_match:
                            verification_code = code_match.group(1)
                            print(f"[DEBUG] Найден код подтверждения: {verification_code} (шаблон: {pattern})")
                            return verification_code

                    # Если не нашли код по шаблонам, ищем все 6-значные числа и выводим их
                    all_codes = re.findall(r'\b\d{6}\b', message_content)
                    if all_codes:
                        print(f"[DEBUG] Найдены потенциальные коды: {all_codes}")
                        # Возвращаем первый найденный код
                        return all_codes[0]

                # Если не нашли письма от Instagram или коды в них, ищем среди всех писем
                if not messages_list:
                    print(f"[DEBUG] Письма от Instagram не найдены, ищем среди всех писем")
                    messages = mailbox.fetch(limit=10, reverse=True)
                    messages_list = list(messages)
                    print(f"[DEBUG] Найдено {len(messages_list)} писем")

                    for msg in messages_list:
                        if "instagram" in msg.from_.lower() or "security" in msg.subject.lower() or "verify" in msg.subject.lower():
                            print(f"[DEBUG] Проверяем письмо: {msg.subject} от {msg.from_}")
